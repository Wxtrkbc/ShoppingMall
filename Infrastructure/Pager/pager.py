...
class Pagenation:
    ...
    def generate_str_page(self,):
        ...
        if self.current_page == 1:   # 上一页
            ...
            prev = '<li><a href="javascript:void(0) onclik="ChangePage(%s)">上一页</a></li>' % (self.current_page - 1,)
        list_page.append(prev)

        for p in range(start_page, end_page):  # 1-10
            if p == self.current_page:
                temp = '<li class="active"><a href="javascript:void(0) onclik="ChangePage(%s)">%s</a></li>' % (p, p)
            else:
                temp = '<li><a href="javascript:void(0) onclik="ChangePage(%s)>%s</a></li>' % (p, p)
            list_page.append(temp)

        if self.current_page == self.all_pages:  # 下一页
            nex = '<li class="disabled"><a href="javascript:void(0);">下一页</a></li>'
        else:
            nex = '<li><a   href="javascript:void(0); onclik="ChangePage(%s)">下一页</a></li>' % (self.current_page + 1,)
